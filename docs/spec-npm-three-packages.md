...
Розділити монорепо на три чітко відмежовані npm-пакети всередині теки `npm/`:
...
- Старт: `"private": true` — споживачі лише в межах монорепо. Публікація в registry — окремим рішенням, коли з'явиться зовнішній споживач.
...
- **Conformance у Rust CI:** інтеграційні тести `mt-core` читають fixtures відносним шляхом `../../npm/contract/fixtures` з override через env `MT_CONTRACT_FIXTURES_DIR` (для запусків поза монорепо). Тест: прогнати скан по `cases/<name>/mt/` → порівняти з `expected/scan.json`.
...
Розділення на окремі git-репо (contract / rust / node) стає безпечним, коли `@7n/mt-contract` не має major/minor-змін протягом кількох релізів `@7n/mt` поспіль. До того — вся еволюція контракту атомарними PR у монорепо.
