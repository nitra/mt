...
### 4.2 Тіло (контракт `task.md` за `graph.md`)

Секції — рівно як у файловому контракті `npm/docs/architecture/graph.md` (`## Task` / `## Done when` / `## Check` / `## Inputs`). `## Check` — машинний done/audit-гейт (кожен непорожній рядок — shell-команда, exit 0; команди витягує `signal.rs`), тож секція має бути присутня у шаблоні як якір для адаптерів. (Старі `## Mission`/`## Context` з `docs/mt.md` більше не пишемо.)
