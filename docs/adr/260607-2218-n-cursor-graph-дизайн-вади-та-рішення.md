...
Після gap-аналізу зафіксовано пʼять уточнень архітектури:

- Composite resolved: батьківський composite-вузол не стає resolved неявно; після `children-resolved` оркестратор запускає roll-up run батька відповідно до його `mode`, і батько пише власний `outputs_NNN.md`.
- Pending-audit lifecycle: `pending-audit_NNN.md` вважається обробленим як derived state, якщо існує auditor `run_M.md` з `created_at` пізніше за pending-audit файл.
- `mode: human` у headless-режимі: `mt watch` пропускає такі вузли; людина запускає їх вручну з IDE.
- Merge після аудиту: `mt watch` виступає wrapper-ом, читає сигнал на кшталт `.ncursor-signal` і виконує merge після успішного аудиту.
- Race condition: `mt run --auto` видалено; єдиним оркестратором стає `mt watch`, а post-merge hook лише будить його.

Ці уточнення закривають ризики навколо composite-вузлів, audit queue, human-mode задач і подвійного запуску ready-вузлів.
