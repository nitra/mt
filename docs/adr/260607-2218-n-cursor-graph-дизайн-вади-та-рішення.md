...
Transcript зафіксував спробу закрити дублювання між старим `flow` lifecycle і новим graph/task протоколом через уніфікацію команд і видалення MT file-presence state.

Корисні уточнення для design-record:
- окремий self-check `verify` розглядався як дубль audit-черги;
- `pending-audit_NNN.md` має привʼязуватися до конкретного `outputs_NNN.md`;
- post-merge hook і watch-daemon розглядалися як два режими запуску оркестратора;
- transcript містить суперечливі назви команд через перейменування, тому це оновлення слід трактувати як дизайн-нотатки, а не як точний CLI contract.

## Update 2026-06-07

Gap-аналіз transcript закрив кілька відкритих місць дизайну graph/task orchestration.

Зафіксовані уточнення:
- composite node переходить через derived `children-resolved`, після чого виконується roll-up run батька відповідно до його `mode`;
- `pending-audit_NNN.md` вважається обробленим derived-правилом: існує auditor run з `created_at` пізніше за pending-audit;
- `mode: human` у headless `mt watch` не запускається автоматично: watch пропускає такі вузли, людина стартує їх вручну з IDE;
- merge після успішного аудиту виконує `mt watch` як wrapper, читаючи сигнал виконання;
- race condition між one-shot auto-run і daemon усувається через єдиний оркестратор `mt watch`; `mt run --auto` видаляється або не використовується як паралельний dispatcher.
