...
- Інженер запускається лише після `actor: agent` з `result: failed`, а не після кожного failed-стану будь-якого актора.
- Для інженера передбачена одна спроба, щоб уникнути нескінченного циклу agent → engineer → agent.
- Після `actor: engineer` з `result: failed` система ескалує проблему до людини через notification-механізм.
- `budget_sec` у `task.md` є спільним бюджетом вузла для всіх акторів.
- При вичерпанні `budget_sec` виконання зупиняється і вузол переходить до ескалації/notification.
