...
Цей документ описує механізми координації між автономними AI-агентами, забезпечуючи атомарність та цілісність завдань. Головною гарантією є концепція "авторитетного claims", що забезпечує єдиний контролер для виконання робіт у вузлі. Система підтримує відстеження стану через "run refs" та гарантує надійну передачу роботи між різними хостами за допомогою контрольованого механізму "checkpoint-handoff". Уся публікація результатів здійснюється через захищений, атомарний процес "fenced publish" у гілку `main`.
