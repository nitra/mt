...
| `pi` | *(немає офіційного)* — сторонній `pi-acp@0.0.31` (`svkozak/pi-acp`) бриджить `pi --mode rpc` до ACP; це саме той пакет, що його для Pi використовує офіційний ACP Registry Zed (`zed.dev/acp/agent/pi` → `npx pi-acp@0.0.31`) | `initialize`/`session/new` живою сесією ✅; повний хід (prompt → відповідь) **не підтверджено** — дефолтна модель сесії `omlx/gemma-4-e4b-it-OptiQ-4bit` (локальна), а omlx-сервер у середовищі перевірки не піднятий; окремо `pi-acp` одразу після `session/new`, ще до першого prompt, шле `agent_message_chunk` з банером самого `pi` (`pi v0.79.9`) — виглядає як фейкова відповідь агента в клієнті, поки клієнт це не фільтрує |
