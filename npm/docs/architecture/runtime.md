...
**Особливість `pi-acp`: банер `pi` у першій відповіді.** `pi-acp` навмисно ловить нерозпарсюваний як JSON prelude на stdout `pi` (коментар автора: "capture it so the ACP adapter can surface it on session start") і шле його як `agent_message_chunk` одразу після `session/new`, ще до першого prompt — той самий wire-формат, що й у справжньої відповіді. У Zed це не плутається з відповіддю: сесія відкривається до першого повідомлення користувача, і Zed тримає persistent-лістенер нотифікацій на весь час сесії, тож банер стає першим рядком порожнього треду. У `agent-core::AcpClient` (`crates/agent-core/src/acp.rs`) нотифікації читаються **лише всередині циклу конкретного `call()`**; `initialize()`/`new_session()` передають no-op `emit`, а банер приходить *асинхронно після* відповіді на `session/new` — тобто лежить нечитаним у пайпі, поки не почнеться цикл читання наступного `call()`, яким завжди є перший `prompt()` користувача. Тому банер механічно приліплюється до першої реальної відповіді. Не виправлено в цьому PR — потребує дренування pending-нотифікацій в `AcpTurnRunner::open_room` між `session/new` і першим `prompt()`.
