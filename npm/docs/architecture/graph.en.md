...
This document describes an architectural model that implements a recursive task graph with dynamic decomposition. It defines a clear contract for nodes, which can be atomic or composite, ensuring system encapsulation and resilience. A key element is a multi-level state system that manages the task lifecycle from initialization to full resolution or recognition as unresolvable. This guarantees a transparent and atomic data flow throughout the entire execution process.
...
`a.md`/`h.md` are **mutable flags**; they define **who** executes the task; never both at the same time.
...
- **Immutability boundary:** before worktree — unrestricted; after — only new files. `mt done`/`mt audit` check `task.md`/`a.md`/`h.md` against `origin/main` and reject on diff.
- **`deps/`** mirrors the `mt/` structure; `ls -R deps/` + strip `.md` → dep-ids without reading contents. Relative dep links in `## Children` → `mt spawn` resolves them to absolute ones.
...
| File | NNN |
| --- | --- |
| `run_NNN.md` | sequential counter (Nth attempt) |
...
`plan_NNN.md` has separate logic: numbering continues while the node is alive; after `mt kill` + re-init → from `001`.
...
`model_tier` is the executor’s source of truth. Runner resolves the tier into **a concrete model of the selected CLI** through the user-level env `MT_AGENT_CLI_MODEL_MAP[<cli>][tier]` (e.g. codex: MIN→luna / AVG→terra / MAX→sola); a CLI without mapping resolves the model itself according to the user’s subscription, and the tier is always passed as a hint via env `MT_MODEL_TIER` ([runtime.md](runtime.en.md#підписочні-cli-виконавці-agent_cli)).

`agent_cli` (which subscription CLI executes the node) is a **per-node** flag in `a.md` with the user-level default env `MT_AGENT_CLI`. Per-node CLI selection is the cross-program dimension of the [goal](../vision.en.md): a specialized tool per node.
...
The same block is in `plan-rejected_NNN.md` (with `"rejected"` + `## Reason`) and `audit-result_NNN.md`. Approval from CLI on a trusted machine may have no signature (compatibility); the `require_signed_approvals: true` policy makes a signature mandatory.
...
**Composite parent:** wrapper adds `## children` — `ref:` to the current fact of each child (except `export: false`).
...
- **`audit-result_NNN.md`** — written exclusively by the auditor; `{ …, actor: auditor, result: success | failed }` + `## Reasoning` + optional `approved_by` signature. `failed` → node moves to `waiting` (rework, run N+1).
...
- **`run-summary.md`** — generated by the wrapper (LLM) after `run_summary_threshold` failure runs; removed on `mt invalidate`/`mt kill`.
- **`history/`** — does not contain `task.md` → scan ignores it. Global kill archive: `<tasks-root>/.history/<ts>-kill-<path>/`.

## Node States

State is **derived**: lifecycle from node artifacts, runtime ownership from remote claim refs.

| Condition | State |
| --- | --- |
| `task.md` exists, no `a.md`/`h.md` | `unassigned` |
...
| `pending-audit_N` exists, `audit-result_N` missing | `pending-audit` |
| accepted fact exists | `resolved` |
...
- **Accepted Fact** — current `fact_N` (max NNN) without `pending-audit_N` or with `audit-result_N: success`.
- **`failed_streak`** — count(runs with result ∈ execution-failure and NNN > latest accepted fact). `decomposed`/`claim-lost`/`handoff` are not counted.
- **`audit_failed_streak`** — separate counter for consecutively rejected audits; ≥ `audit_retry_max` → escalation to human.
- **Deps satisfaction:** dep must be `resolved`; dep with open audit → `blocked`; dep without fact → `blocked-invalid-dep` (warning).
- **Composite:** state is determined by the atomic ones; when all children are resolved, the wrapper writes a synthetic `run`/`fact` pair (actor: wrapper) — recursively up.
...
context = [task.md] + [a.md|h.md] + [deps/] + [plan_*.md] +
          [Prior attempts summary] + [run-summary.md if present] + [audit-result_*.md]
```

**Prior attempts summary** — two layers of failure compression: wrapper compacts all failure-runs into a summary (Completed/Blockers/Next Attempt); after `run_summary_threshold` failure-runs — additionally `run-summary.md` (LLM summary, second layer). During operation, the agent maintains `run-draft.md`; upon completion, the wrapper moves sections to `run_NNN.md`.
...
**Spawn Protocol:** `mt spawn --approve` validates `## Children` (naming, mode per-child, deps exist, no cycles), materializes children, commits `plan-approved_NNN.md` + children files in **one fenced atomic commit**. Node is legitimate ↔ its id in the parent's approved plan's `## Children` or it is root; other directories with `task.md` → `orphan-node` warning. **Dynamic decomposition** during Stage 2: agent writes `plan_NNN+1.md` (composite) → `result: decomposed` → `plan-review`.

**Node Patch Protocol:** `mt stop` successors (from list) → `mt invalidate <target>` → patch → fenced publish → runner picks up descendants. `mt invalidate` archives version chain in `history/`; after re-run, compare the hash of the new fact: same → descendants unblock; different → cascade invalidate downwards. `mt kill` — only final deletion of the subtree from topology.

## Retry ladder, engineer, unresolvable

Until `agent_retry_max` (3), the node remains `waiting`; the agent retries according to the ladder (`MT_ATTEMPT` = `failed_streak` + 1): 1 — base; 2 — diagnose-first; 3 — alternative-approach (`model_tier: +1`, `skills_add`). Shorter ladder → last step is repeated.
...
**Unresolvable:** (1) streak ≥ `agent_retry_max + engineer_retry_max`; (2) `plan-rejected` ≥ `plan_reject_max`; (3) `sum(wall_sec) > budget_total_sec` → `unresolvable.md` + alert (relay push to owner). Output — human: `mt invalidate` (+ edit task.md), `mt kill`, or `mt run <ancestor> --actor engineer`.

## Audit (Async Queue)

Blocking gate: node with an open audit = `pending-audit`; dependents wait. Triggers: `mt audit`, `audit_schedule_days`, `audit_on_patch`.
...
The auditor can be an agent (`audit_model`) **or a human with the `approver+` role** — then the verdict is signed via relay (see [access.md](access.en.md)), and the host materializes `audit-result_NNN.md` with a signature block.
...
1. **Initialization.** `mt init quarterly-anomalies --task "Investigate Q4 payment anomalies" --mode agent --budget-sec 3600` → `task.md` + `a.md`; human adds `## Done when` and `## Check`.
2. **Inline Plan → Composite.** Orchestrator sees `waiting` → runner: CAS claim → worktree → agent writes `plan_001.md` (decision: composite) with three children: `collect-data` (agent), `analyze` (agent, `audit: required`, deps: collect-data), `review-findings` (human, deps: analyze). Run completes `result: decomposed` → node in `plan-review`.
3. **Approve.** `mt spawn --approve` validates `## Children` → materializes children + `plan-approved_001.md` in one fenced commit. Parent → `spawned`.
4. **collect-data — happy path.** claim → inline `plan_001` (atomic) → `fact_001.md` → `mt done` → `## Check` pass → `run_001` (success) → fenced publish → `resolved`.
5. **analyze — retry ladder + audit.** Attempt 1 fails (`run_001` failed, `failed_streak = 1`) → retry `MT_ATTEMPT=2` (diagnose-first) → success: `fact_002.md`; `audit: required` → `pending-audit_002.md`. Auditor doubts → `clarification_002.md` → agent `--amend` → `amended_002.md` → re-audit → `audit-result_002.md` (success) → `resolved`. During this time, `review-findings` is **blocked** (dep not resolved — blocking gate).
6. **Human Node.** `review-findings` → `pending` + push assignee → human reviews → `fact_001.md` → `mt done` → `resolved`.
7. **Composite Aggregation.** All children resolved → wrapper writes a synthetic `run_001`/`fact_001` pair (actor: wrapper) with `## children`-refs → parent `resolved`.

### Part 2 — Same in Target View (sessions, devices, languages)

8. **Failed Node Picked Up in Chat.** Assume both attempts of `analyze` failed at step 5. The analyst on the laptop runs `mt attach mt/quarterly-anomalies/analyze/` — this is a **new run of the same node** ([runtime.md](runtime.en.md)): CAS claim (`interactive: true`), worktree, replay. Contract unchanged — graph sees a normal run.
9. **Materialization in User's Language.** For the analyst, `lang: uk` — README and task content are materialized in Ukrainian in their worktree ([i18n.md](i18n.en.md)); their note correction is compiled into the base before publish.
10. **Handoff.** Handling the data is too hard for the laptop — "move it here" to the server: the holder finishes the turn, writes `run_NNN (result: handoff)`, CAS-delete; the server creates a claim (generation+1), inherits the log ([git.md](git.en.md)). The client-laptop remains connected to the same room.
11. **Signed Approval from Phone.** The agent wants to perform a destructive step (writing to prod-DB) → `ApprovalRequest` → the second participant's phone (role `approver`, **without git access**) shows the diff → Ed25519 signature → line in `## Approvals` run-file ([access.md](access.en.md)).
12. **Completion.** `mt done` → `## Check` → fenced publish; next — steps 5–7 as in part 1. The graph state in `main` is unchanged from the autonomous run — sessionability and languages leave no trace in the canonical state.
