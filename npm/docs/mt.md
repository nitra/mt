...
Watch перевіряє: `ts + grace_period ≤ now()` (grace_period: 60s — конфіг у `.mt.json`).
...
- **`--auto` / без `path`**: one-shot оркестратор — знаходить всі `waiting` вузли з `a.md`; немає плану → auto `mt plan`; є план → auto `mt run`; spawns через mkdir lock; `pending` (h.md) і `unassigned` — пропускає
- **`--actor auditor`**: wrapper аудитора — spawns auditor subprocess, чекає, читає `audit-result_NNN.md`, success → merge
- **`--actor human`**: створює worktree і виводить шлях; людина працює вручну у цьому worktree; по завершенні викликає `mt done|audit|failed` сама; wrapper пише `run_NNN.md` (actor: human) після сигналу
...
- Послідовність:
  ...
  4. `git rm -r mt/<path>/` + `git commit "mt: kill mt/<path>"`
...
**`mt run --auto`** (one-shot, тригерується post-merge hook) і **`mt watch`** (periodic rescan) — обидва шукають `waiting` вузли з `a.md`. Координація через `mkdir .worktrees/<node>-<hash>/` — атомарна FS-операція; перший runner отримує директорію, другий — `EEXIST` → skip.
...
mt watch (periodic, кожні 5 хв або по wake):
  ...
  → waiting вузли з a.md → немає плану: auto mt plan; є план: auto mt run
      ...
      mkdir .worktrees/<node>-<deadline-unix>/
        EEXIST  → skip
        success → mt run --actor agent (при відсутності plan_*.md → auto-планує перед run)
...
mt watch:
  ...
  → mt run --actor auditor <path>
      wrapper: новий worktree з main (читає fact_NNN.md + pending-audit_NNN.md + amended_NNN.md якщо є)
        ...
        success → merge + delete auditor worktree + touch .mt/wake
...
> **Startup cleanup:** При старті `mt run <path>`: якщо є `running_<pid>_until_<ts>` → виконати `kill -0 <pid>` на PID з імені файлу. Якщо процес мертвий → cleanup running\_\* + worktree → продовжити старт. Якщо живий + `ts > now()` → skip (справді running). На distributed FS: якщо `ts + stale_grace_period_sec ≤ now()` → cleanup.
...
4. `git worktree add .worktrees/<sanitized-path>-<epoch> main` (атомарний mkdir-lock)
   4a. Пише `mt/<node>/running_<pid>_until_<started_at + budget_hard_sec>` (git-ignored sentinel; ts = unix epoch UTC)
...
9. `result: success` → **merge protocol** → видаляє `running_<pid>_until_*` → `touch .mt/wake`
...
5. Читає `result`:
   - `success` → **merge protocol** → `touch .mt/wake`
...
**Чому conflicts неможливі:** кожен вузол пише виключно у свою піддиректорію `mt/<node>/`. Паралельні merge різних вузлів ніколи не торкаються спільних файлів.
...
- **TODO (distributed):** clock skew при multiple runners на різних хостах — `running_<pid>_until_<ts>` перевірки можуть бути некоректні. WON'T FIX до distributed setup; при потребі: `grace_period_sec` у `.mt.json`.
...
| ENV для агента                 | `MT_BUDGET_SEC`, `MT_HARD_BUDGET_SEC`, `MT_STARTED_AT`, `MT_RUN_NNN`                                                                                                              |
...
| Auditor worktree               | Новий worktree з main (агент мержить при `mt audit`)                                                                                                                                               |
...
| Merge після аудиту             | `mt run --actor auditor` wrapper; success → git merge + delete worktree                                                                                                                               |
...
| Моніторинг                     | `mt scan` — відновлює граф зі стану файлів в будь-який момент                                                                                                                                         |
