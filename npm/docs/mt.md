---
type: contract
description: 'Повна специфікація графа задач, яку реалізує поточний @7n/mt (deprecated як цільова картина)'
tags: [contract, graph, cli, publish]
...
>
> 🧊 **Frozen contract 0.2.x.** Документ зафіксовано і **не редагується**: це чинний контракт поточної реалізації `@7n/mt`. Канон розвитку — [architecture/](architecture/index.md) (0.3.0-draft); правки архітектури сюди **не** синхронізуються. Документ буде видалено на milestone M1 ([roadmap.md](roadmap.md)), коли runtime-шар реалізує протокол v4. Зміст документації — [index.md](index.md).
