...
`schema_version:` — перше поле у всіх файлах з YAML-фронтматером. Поточна версія: `1`. Оркестратор підтримує **backward compatibility**: читає всі версії, які він знає (включаючи попередні).

**Поведінка при невідомій версії (fail closed):** якщо `schema_version` вищий за власний максимум оркестратора → **відмовляє файл, не пропускає мовчки**. Пише у stderr: `[mt] FATAL: <path>: schema_version=N > supported=M — оновіть mt`. Вузол переходить у `stalled` доки файл не стане читабельним (оновлення оркестратора або ручне виправлення файлу).

**Невідомі поля:** оркестратор ігнорує додаткові поля (forward compatibility у межах тієї ж major версії), але пише у stderr попередження у debug-режимі: `[mt] WARN: <path>: unknown field '<field>' (schema_version=N)`. Ніколи не ігнорує мовчки при `--verbose`.

Breaking schema changes постачаються як окремий major release MT з явним описом переходу і migration script; новий orchestrator читає і стару, і нову версію без `migration_state`.
...
`result` — повний enum: `success | failed | progress-timeout | budget-exceeded | claim-lost | merge-conflict | decomposed`. Результати розбиті на три категорії:

| Категорія | Values | `fact_NNN.md` | `failed_streak` |
|---|---|---|---|
| Terminal success | `success` | створюється | скидається |
| Execution failure | `failed`, `progress-timeout`, `budget-exceeded`, `merge-conflict` | не створюється (дірка) | **+1** |
| Lifecycle / ownership | `decomposed`, `claim-lost` | не створюється | **не змінюється** |
...
**`failed_streak`** — лічильник поточної серії execution failures:

```
last_accepted_NNN = max(NNN серед fact_*.md де fact є прийнятим; 0)
  (прийнятий fact: немає pending-audit_NNN, або audit-result_NNN: success)

failed_streak = count(run_*.md де result ∈ {failed, progress-timeout,
               budget-exceeded, merge-conflict} і NNN > last_accepted_NNN)
```

`decomposed` і `claim-lost` у streak не входять. Anchor — **останній прийнятий** fact, не просто будь-який: fact, відхилений аудитом, не скидає streak. Оркестратор читає поле `result:` з frontmatter `run_NNN.md` і `audit-result_*.md` при скані. `mt invalidate` архівує весь version chain → streak скидається з новою chain.

**`audit_failed_streak`** — окремий лічильник поспіль відхилених аудитів:

```
audit_failed_streak = count consecutive audit-result_*.md (result: failed)
                      з найбільшого NNN до першого success або початку chain
```

Не пов'язаний із `failed_streak`. Ескалація — окрема: `audit_failed_streak ≥ audit_retry_max` → watch ескалює на людину (Telegram). Дефолт `audit_retry_max: 3` (поточне "3 поспіль" у логіці watch).
...
Переходи:
- `plan-review` → `spawned` (`mt spawn --approve`: валідація `## Children` + матеріалізація дітей + `plan-approved_NNN.md`)
- `plan-review` → `waiting` (`mt spawn --reject --reason "..."`: пише `plan-rejected_NNN.md`; `plan_NNN.md` зберігається для аудиту; наступний `mt plan` отримує контекст відхилення)
...
**Ліміт аудит-циклів (`audit_failed_streak`):** після `audit_retry_max` (дефолт: 3) поспіль `audit-result_*.md (result: failed)` — watch ескалює на людину (Telegram/email). Окремо від `failed_streak` — execution failures і audit rejections мають незалежні ескалаційні шляхи.
...
- Знаходить вузли де `count(plan-rejected_*.md) ≥ plan_reject_max` → пише `unresolvable.md` (причина: plan disagreement) + алерт людині
...
```
# Тригер 1: execution failures вичерпані
вузол X: failed_streak ≥ agent_retry_max + engineer_retry_max
  → watch пише unresolvable.md (причина: "execution failures exhausted")
  → алерт людині (Telegram/email)

# Тригер 2: агенти не домовились по плану
вузол X: count(plan-rejected_*.md) ≥ plan_reject_max
  → watch пише unresolvable.md (причина: "plan rejected too many times — human arbitration needed")
  → алерт людині (Telegram/email)
  (failed_streak не враховується — це не execution failure)
...
`agent_retry_max` — скільки поспіль execution failures (`failed_streak`) дозволено агенту, перш ніж вузол перейде у `failed` і його забере EngineerAgent. `decomposed` і `claim-lost` у streak не входять. `engineer_retry_max` — скільки спроб має інженер до `unresolvable`. `retry_ladder` — драбина диверсифікації ретраїв (див. «Політика ретраїв»).

`plan_reject_max` — скільки разів план вузла може бути відхилений (reviewer-агентом або людиною) до ескалації на людину через `unresolvable`. Захист від нескінченного циклу декомпозицій між агентами. Дефолт: `3`.
