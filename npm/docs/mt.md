...
- Послідовність:
  1. Архівує весь вузол і нащадків: `<tasks-root>/.history/<ts>-kill-<node-path>/`
  2. Мутація (`git rm -r mt/<path>/` + commit "mt: kill mt/<path>") публікується через **fenced publish protocol** — atomic push з force-with-lease на `main`; protected `main` → integration branch + PR (approval-only) + fenced bot push
...
- GC: failure run refs старші за `run_ref_ttl_days` (дефолт 14) → видаляє remote ref; артефакти вузла вже опубліковані в `main`, ref існує лише для debug
...
mt/quarterly-anomalies/
  ...
  analyze/         task.md  a.md  deps/collect-data.md  plan_001.md
                   run_001.md (failed)  run_002.md  fact_002.md
                   pending-audit_002.md  clarification_002.md  amended_002.md  audit-result_002.md
  review-findings/ task.md  h.md  deps/analyze.md  run_001.md  fact_001.md
```
