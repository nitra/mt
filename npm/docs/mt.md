...
failed_streak = count(run_*.md де result ∈ {failed, progress-timeout,
               budget-exceeded, merge-conflict} і NNN > last_fact_NNN)

де last_fact_NNN = max(NNN серед fact_*.md; 0 якщо fact немає)
```

`decomposed` і `claim-lost` у streak не входять. Оркестратор читає поле `result:` з frontmatter кожного `run_NNN.md` при скані (виняток із правила «без читання вмісту» — поруч із `audit-result_*`). `mt invalidate` архівує весь version chain → streak скидається з новою chain.
