...
- Послідовність:
  ...
  6. Мутація (`git rm -r mt/<path>/` + commit "mt: kill mt/<path>") публікується через **fenced publish protocol** — atomic push з force-with-lease на `main`; protected `main` → integration branch + PR (approval-only) + fenced bot push
